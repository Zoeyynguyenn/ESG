#!/usr/bin/env python3
"""Complete partial hotspots + family extraction generalization + light holdout expansion."""
...
PRIOR_BASELINE = ROOT / "reports/enterprise_docs_registry_holdout_20260618-160935/summary.json"
...
def _family_delta(
    ...
    if bucket not in pf:
        return None
    return round(
        cf.get("extraction_feasible_rate", 0) - pf[bucket].get("extraction_feasible_rate", 0),
        4,
    )
...
def _system_decision_gate(
    ...
    prior_ext = None
    if prior:
        pb = prior.get("holdout_summary", {}).get("by_company") or prior.get("holdout_summary", {})
        if isinstance(pb, dict) and "hanssem" in pb:
            prior_ext = (
    ...
    env = (family_view.get("families") or {}).get("environment_ghg") or {}
    emp_hc = (family_view.get("families") or {}).get("employee_headcount") or {}
    ...
    return {
        "ready_for_holdout_expansion": h_ret >= 0.75 and m_ext >= 0.3 and ext_avg >= 0.35,
        ...
        "employee_headcount_extraction_rate": emp_hc.get("extraction_feasible_rate"),
...
def _write_report(path: Path, payload: dict[str, Any]) -> None:
    ...
    hr = payload.get("hotspot_reduction") or {}
    ...
    lines = [
        ...
        f"- Pilot-only labeled: **{mig['counts'].get('pilot_only_labeled', 0)}**",
        "",
        f"Partial hotspots reduced: **{hr.get('partial_before')} → {hr.get('partial_after')}**",
        "",
        ...
        f"- Governance extraction: **{gate.get('governance_extraction_rate')}** (still weakest: {gate.get('governance_still_weakest')})",
        f"- Environment GHG extraction: **{gate.get('environment_ghg_extraction_rate')}** (delta {gate.get('environment_ghg_delta_vs_prior')})",
        "",
        "## Family summary",
        "",
    ]
...
def main() -> int:
    ...
    out_dir = ROOT / args.reports_dir / f"enterprise_docs_family_holdout_{ts}"
    ...
    if prior:
        ...
        baseline_delta = {
            ...
            "probe_count_delta": {
                "hanssem": (cb.get("hanssem") or {}).get("probe_count", 0)
                - (pb.get("hanssem") or {}).get("probe_count", 0),
                "musinsa": (cb.get("musinsa") or {}).get("probe_count", 0)
                - (pb.get("musinsa") or {}).get("probe_count", 0),
            },
        }
